...
table**. Each element is converted to its discrete logarithm with respect to a
primitive element, the logs are added, and the result is converted back. This
turns multiplication into addition on a cyclic group of order 2^m minus 1.
...
- `analysis/check_gf16_logsum_frequencies.py`: tests output logits for causal log-sum frequency structure (Fourier and ablation)
...
discrete-log order (top-2 concentration around 0.16 either way).
...
frequency concentration in discrete-log order (around 40% of power in the top 4
frequencies) versus natural order (around 8.7%). Removing the dominant
frequencies causally collapses accuracy (100% down to around 4% at k=16
frequencies removed), while removing an equal number of random frequencies
leaves accuracy untouched (100% throughout). This confirms that the structure
...
ratio of around 4.6 to 6.2% (consistent across 3 independently trained seeds)
versus around 93.9 to 99.6% for a random-grouping control. In other words,
knowing only the log-sum explains nearly all of the model's output variation.
...
also computationally heavier to scale to larger fields (a full per-class 2D FFT
and ablation sweep). `logsum_dependence.py`, the most direct and cheapest test
of the core hypothesis, was instead run across every field size tested, to
check whether the finding holds as field size grows.
...
memorization plateau, meaning no classic grokking.
...
here too, and even more cleanly than at GF(16): a ratio of around 0.79 to 0.9%
versus a 99.6% random control.
...
seed=0. Train and test rose together, with only a small, brief lag (max gap
around 0.08 to 0.12) rather than a sustained plateau.
...
- **m=4 (GF16), m=5 (GF32):** clear, dramatic grokking, with a long flat
  plateau in test accuracy followed by a sharp jump. Consistent across seeds.

  ![GF(32) sweep](results/run_gf32_seeds_0_1_curve.png)

- **m=6 (GF64), m=7 (GF128):** also clear grokking, comparable in character
  to m=4 and m=5. (One m=7 seed showed an interesting two-stage climb,
  suggesting a possible intermediate or partial solution before full
  convergence; noted but not further investigated.)
...
- **m=9 (GF512):** `experiments/run_gf512_partial.py`, a partial, unconverged
  check (1500 steps, final train/test accuracy around 39%/35%). The shape so
  far shows train and test rising almost together, with a small, fairly
  steady gap (around 0.05), consistent with the shrinking-gap trend
...
m=4 through m=7, then shrinks substantially starting at m=8, with m=9's
partial data consistent with further shrinkage. This is consistent with the
standard account of why grokking happens. A model can easily memorize a small
training set before weight decay forces it toward the generalizing solution.
